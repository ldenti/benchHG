        ...
        for s in sources:
            for t in targets:
                for path in nx.all_simple_paths(G, source=s, target=t):
                    compatible_haplos.append(path)

        # TODO: get only haplotypes coherent with GT
        for ph in compatible_haplos:
            self.candidates_haplotypes_sequences.append([])
            self.candidates_haplotypes.append([])

            # here we build a variation graph
            G = nx.DiGraph()
            last_nodes = []
            last_p = self.ustart
            n = 0
            for v in ph:
                record = self.records[v]
                ...
                n += 1
                curr_last_nodes = []
                for i, a in enumerate(record.alleles):
                    G.add_node(n, v=v, seq=a.upper(), gt=i)
                    for node in last_nodes:
                        G.add_edge(node, n)
                    curr_last_nodes.append(n)
                    n += 1
                last_p = record.stop
                last_nodes = curr_last_nodes
            G.add_node(n, v=-1, seq=seq[last_p : self.uend + 1].upper(), gt=0)
            for node in last_nodes:
                G.add_edge(node, n)
            # nx.nx_pydot.write_dot(G, sys.stdout)

            # CHECKME: is it true that we cannot have isolated nodes?
            sources = [
                n for n in G.nodes() if G.in_degree(n) == 0 and G.out_degree(n) > 0
            ]
            targets = [
            ...
            for s in sources:
                for t in targets:
                    for path in nx.all_simple_paths(G, source=s, target=t):
                        ...
                        for n in path:
                            cseq += G.nodes[n]["seq"]
                            chap.append((G.nodes[n]["v"], G.nodes[n]["gt"]))
                        if sum([ch[1] for ch in chap]) == 0:
                            continue
                        # if truth and not all(
                        #     [ch[1] for ch in chap[1:-1:2]]
                        # ):  # [1:-1:2] just take variations
                        #     continue
                        self.candidates_haplotypes_sequences[-1].append(cseq)
                        self.candidates_haplotypes[-1].append(chap)
...
def main(args):
    ...
    # TODO: plot sizes of haploblocks_1 and haploblocks_2
    ...
    for call_hb in track(haploblocks_2):
        ...
        if len(hits) == 0:
            ...
            if len(truth_hb.records) > 5 or len(call_hb.records) > 5:
                # TODO: build sequences, then filter "same" haplotype?
